...
export function requireVerifiedSellerAccess(req, _res, next) {
 ...
 if(!req.auth.email_verified) throw new HttpError(403,'Verify your email before listing products.');
 if(!['seller','admin'].includes(req.auth.role)) throw new HttpError(403,'Complete your selling settings first.');
 next();
}
