...
function senderParts() {
  ...
  if (match) {
    ...
    return {
      name: name || env.smtpFromName || "Gleenc",
      email: match[2].trim(),
    };
  }

  return {
    name: env.smtpFromName || "Gleenc",
    email: raw.trim(),
  };
}
...
export async function sendDiagnosticEmail({ to }) {
  return sendTransactionalEmail({
    to,
    subject: "Gleenc email delivery diagnostic",
    text: `This is a Gleenc email diagnostic sent at ${new Date().toISOString()}.
    ...
    html: `
      <div style="font-family:Inter,Arial,sans-serif;background:#f8fafc;padding:28px;color:#0f172a;">
        <div style="max-width:560px;margin:0 auto;background:#ffffff;border:1px solid #e2e8f0;border-radius:22px;padding:28px;">
          <p style="margin:0 0 10px;color:#16a34a;font-size:12px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;">Gleenc diagnostic</p>
          <h1 style="margin:0 0 12px;font-size:26px;line-height:1.15;color:#020617;">Email delivery is connected</h1>
          <p style="margin:0;color:#475569;font-size:15px;line-height:1.6;">This message confirms that the deployed Gleenc backend can connect to Brevo and send transactional emails.</p>
        </div>
      </div>
    `,
  });
...
export async function sendEmailVerificationEmail({ to, name, token }) {
  ...
  await sendTransactionalEmail({
    to,
    subject: "Verify your Gleenc account",
    text: `Hello ${name || "there"},

Verify your Gleenc account by opening this link:
${verificationUrl}

This link expires soon. If you did not create a Gleenc account, ignore this email.`,
    html: `
      <div style="font-family:Inter,Arial,sans-serif;background:#f8fafc;padding:28px;color:#0f172a;">
        <div style="max-width:560px;margin:0 auto;background:#ffffff;border:1px solid #e2e8f0;border-radius:22px;padding:28px;">
          <p style="margin:0 0 10px;color:#16a34a;font-size:12px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;">Gleenc verification</p>
          <h1 style="margin:0 0 12px;font-size:26px;line-height:1.15;color:#020617;">Verify your email</h1>
          <p style="margin:0 0 18px;color:#475569;font-size:15px;line-height:1.6;">Hello ${safeName}, confirm this email address to secure your Gleenc account and unlock protected actions.</p>
...
export async function sendPasswordResetEmail({ to, name, token }) {
  ...
  await sendTransactionalEmail({
    to,
    subject: "Reset your Gleenc password",
    text: `Hello ${name || "there"},

Reset your Gleenc password by opening this link:
${resetUrl}
...
This link expires soon. If you did not request a password reset, ignore this email.`,
    html: `
      <div style="font-family:Inter,Arial,sans-serif;background:#f8fafc;padding:28px;color:#0f172a;">
        <div style="max-width:560px;margin:0 auto;background:#ffffff;border:1px solid #e2e8f0;border-radius:22px;padding:28px;">
          <p style="margin:0 0 10px;color:#16a34a;font-size:12px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;">Gleenc security</p>
          <h1 style="margin:0 0 12px;font-size:26px;line-height:1.15;color:#020617;">Reset your password</h1>
          <p style="margin:0 0 18px;color:#475569;font-size:15px;line-height:1.6;">Hello ${safeName}, use this secure link to choose a new password for your Gleenc account.</p>
